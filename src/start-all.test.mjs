...
assert.match(script, /dd-adapter-crush/);
assert.match(script, /dd-adapter-hermes/);
...
assert.match(script, /local devspace_secret="\$\{DEVDESKTOP_ACP_SHARED_SECRET:-\$\{DEVDESKTOP_ACP_AGENT_SECRET:-\$\{DEVDESKTOP_ACP_REVIEWER_SECRET:-\}\}\}"/);
assert.match(script, /Authorization: Bearer \$\{devspace_secret\}/);
